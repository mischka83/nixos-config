...

{
  ...
  systemd.user.services.flatpak-bootstrap = {
    Unit = {
      ...
      ConditionPathExists = "!%h/.local/state/flatpak-bootstrap-v2.done";
    };
    ...
    Service = {
      ...
      ExecStart = pkgs.writeShellScript "flatpak-bootstrap" ''
        ...
        ${pkgs.flatpak}/bin/flatpak --user uninstall -y --noninteractive --delete-data net.devolutions.RDM || true
        ${pkgs.flatpak}/bin/flatpak --user uninstall -y --noninteractive --delete-data com.github.IsmaelMartinez.teams_for_linux || true
        ...
        : > "$HOME/.local/state/flatpak-bootstrap-v2.done"
      '';
    };
